...
void updateST(int start,int end,int si,int index,int diff)
{
    if(index<start || index>end) //index is out of current node range
        return;
    
    segTree[si]=segTree[si]+diff;
    
    if(start<end) //have child nodes
    {
        int mid=(start+end)/2;
        updateST(start,mid,si*2+1,index,diff);
        updateST(mid+1,end,si*2+2,index,diff);
    }
}
